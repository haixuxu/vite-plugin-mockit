...
describe('mock api', function () {
  ...
  it('should can mock object vlaue', function (done) {
    const result = { username: 'admin', sex: 5 };
    request(server).get('/api/user').expect(200, result, done);
  });
  it('should can mock with query params', function (done) {
    ...
    request(server).get('/api/list?limit=10&offset=0').expect(200, result, done);
  });
  ...
  it('should can accept POST params', function (done) {
    ...
    request(server).post('/api/login/account').send({ username: 'admin', password: '888888' }).expect(200, data, done);
  });
  it('should can accept DELETE params', function (done) {
    request(server).delete('/api/user/122').expect(200, { status: 'ok', message: '删除成功！', id: 122 }, done);
  });
